...
public class Cmd_DrivetoCharge extends CommandBase {
    ...
    public Cmd_DrivetoCharge(DriveSubsystem drive) {
        m_driveTrain = drive;
    ...
    public void execute() {
        ...
        double DriveSpeed = BalancePID.calculate(m_driveTrain.getRoll());
        if ((maxAngle - m_driveTrain.getRoll()) > 1)
            DriveSpeed = 0;
        DriveSpeed = MathUtil.clamp(DriveSpeed, -1, 1);
        
         m_driveTrain.drive(DriveSpeed, 0, 0, false);  
    ...
    public boolean isFinished() {
        ...
        if (m_driveTrain.getRoll() > 11 && currenttime > 1.5)
        {
            SmartDashboard.putString("Drive Charge","DriveCharge finishe");
                return true;
        }
        if (m_driveTrain.getRoll()<-5)
            return true;
        return false;
    }
