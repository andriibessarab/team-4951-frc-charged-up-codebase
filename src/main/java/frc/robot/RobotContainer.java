...
public class RobotContainer {
        ...
        m_robotDrive.setDefaultCommand(
            ...
            new RunCommand(() -> {
                    ...
                    m_robotDrive.driveMecanum(
                        MathUtil.applyDeadband(controllerLeftX, Constants.OIConstants.DriverControl.kDriveDeadband), MathUtil.applyDeadband(-controllerLeftY, Constants.OIConstants.DriverControl.kDriveDeadband), MathUtil.applyDeadband(controllerRightX * 0.67, Constants.OIConstants.DriverControl.kRotationDeadband));
                },
                m_robotDrive));
    ...
    private void configureButtonBindings() {
        ...
        new JoystickButton(m_driverController, Button.kA.value).onTrue(new AutoSQ_NewBalance(0, m_robotDrive));
        ...
        new JoystickButton(m_operatorController, Button.kY.value) // Xbox kY
            .onTrue(new ElevatorGotoPosition(m_elevator, Constants.ElevatorSubsystemConstants.kTopLayerHeight));

        // Move elevator to middle layer
        new JoystickButton(m_operatorController, Button.kB.value) // Xbox kB
            .onTrue(new ElevatorGotoPosition(m_elevator, Constants.ElevatorSubsystemConstants.kMidLayerHeight));

        // Move elevator to bottom layer
        new JoystickButton(m_operatorController, Button.kA.value) // Xbox kA
            .onTrue(new ElevatorGotoPosition(m_elevator, Constants.ElevatorSubsystemConstants.kBottomLayerHeight));
        ...
        new JoystickButton(m_operatorController, Button.kLeftStick.value) // Xbox kLeftStick
            .onTrue(new ArmGoToPosition(m_arm, Constants.ArmSubsystemConstants.kMinExtend));

        // Extend arm
        new JoystickButton(m_operatorController, Button.kRightStick.value) // Xbox kRightStick
            .onTrue(new ArmGoToPosition(m_arm, Constants.ArmSubsystemConstants.kMaxExtend));
    ...
    public Command getAutonomousCommand() {
        return new SequentialCommandGroup(
            ...
            new ElevatorGotoPosition(m_elevator, Constants.ElevatorSubsystemConstants.kTopLayerHeight), new PivotGoToPosition(m_pivot, 1.7), new ClawOutake(m_claw), new PivotClose(m_pivot), new ElevatorGotoDown(m_elevator, 0.0), new LeaveCommunityZone(m_robotDrive)
            //new 
