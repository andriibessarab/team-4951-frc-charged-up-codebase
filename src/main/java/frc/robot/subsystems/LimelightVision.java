...
public final class LimelightVision extends SubsystemBase {
     ...
     * 
     * @see <a href=
     *      "https://docs.limelightvision.io/en/latest/networktables_api.html">Limelight
     *      NetworkTables API</a>
     */
    ...
    /**
     * Creates a number Array to store the robot position values from the botPose
     * entry
     ...
     * 
     * @return returns which pipeline is currently used. returns -1 if no pipeline
     *         selected.
     */
    ...
    /**
     * Returns the total area that the current target takes up on the limelight's
     * screen
    ...
    /**
     * checks if the limelight is in Apriltag, and if it has a target, returns the
     * ID of the Apriltag. Otherwise, it returns null.
     * 
     ...
     * 
     * @return The horizontal offset angle from the limelight crosshair to the
     *         target
     */
     ...
     * 
     * @return The skew of the target that is currently within the limelight's
     *         viewframe
     */
    ...
    /**
     * Uses the pitch and yaw values from networkTables to construct and returns a
     * rotation2D
     * 
     * @return A rotation2D made from the robot pitch and yaw values, represents
     *         rotation to the currently seen target
     */
    public final Rotation2d createRotation2D() {

        /**
         * Gets the pitch value from the robotPositionValues array & converts it to
         * radians
         */
        ...
        /**
         * Gets the yaw value from the robotPositionValues array & converts it to
         * radians
         */
     ...
     * 
     * @return a translation2D made from the robot x and robot y values, represents
     *         movement to the currently seen target
     */
     ...
     */
    public final Transform2d createTransform2D(Translation2d constructorTranslation2d,
            Rotation2d constructorRotation2d) {
        /**
         * Uses a translation2d & a rotation2d parameter to construct a transform2d,
         * then returns it
         */
    ...
    /**
     * Checks what target we are looking at, calculates the forward distance, and
     * returns it
     * 
    ...
    public final Double forwardDistanceToTarget() {
        if (hasTargets()) {
            ...
            if (verticalOffset > LimelightConstants.MAX_VERT_OFFSET_FOR_LOW
                    && verticalOffset <= LimelightConstants.MAX_VERT_OFFSET_FOR_MED) {
                double horizontalFromMed = LimelightConstants.HEIGHT_TO_MED / Math.tan(verticalOffset);
                return horizontalFromMed;
            }

            if (verticalOffset > LimelightConstants.MAX_VERT_OFFSET_FOR_MED
                    && verticalOffset <= LimelightConstants.MAX_VERT_OFFSET_FOR_HIGH) {
